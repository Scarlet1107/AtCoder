...
int main() {
    ...
    for(i = 0; i < SIZE; i++) index[i] =0; // Initialize (
    ...
    i = num_of_chars;
    construct_2(data,i); //出現数を持った文字たちをヒープに入れる（「親は子より出現頻度の値が小さい」という順序関係に基づいてヒープ化する＝
            // minimum heap）
    ...
    while(i > 1) {
        ...
        newnode.count = couple[0].count+couple[1].count; // 取り出した2つの節点の値（出現回数）の合計
