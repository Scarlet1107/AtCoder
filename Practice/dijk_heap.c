...
void Dijkstra(int root, int n) {
    ...
    while (visited_order < n) {  // S !=V  -->  |V| < |S|
        ...
        if (min_cost == INFTY) continue;  // no edges

        // Chose a vertex w in  V-S
        if (label[min_node] != 0) continue;
        // add w to S;
        ...
        for (i = 0; i < n; i++) {
            ...
            if (/* [ Complete Here!! ] */) {
                if (d[i] == INFTY) {
                    ...
                    newnode.dist = d[i];
                    newnode.key = i;
                    replace(data, newnode, ind[i + 1]);  // 既存の値より短い辺が出現（書き換えた場合）
                }
            }
        }
    }
...
void downheap(DIST *A, int k, int n) {
    ...
    while (k <= n / 2) {
        j = k + k;
        if (j < n && A[j].dist > A[j + 1].dist) j++;
        if (tmp2 <= A[j].dist) break;
...
/*
  ...
  ヒープが壊れる場所が p に変わる
  minimum heap であることとreplaceが呼ばれるときの状況を考えると，子に対しては親子関係が成立するも，親に対しては成立しない可能性がある．
  なので，upheapを使う．
*/

void replace(DIST *A, DIST v, int p) {
    A[p] = v;
    upheap(A, p);
}
